...
- Opt-in feed reachability tracking via `reachability: true` in `feeds.initialize()`. Reports whether the browser has internet, whether the user is live on the feed socket and whether the client can reach the feed notifications API, as a combined `ONLINE | CONNECTING | DEGRADED | AUTH_ERROR | OFFLINE | UNKNOWN` status.
